...
| `figma_bridge_codegen` | Sinh **khung JSX (React inline-style)** cho 1 node (gộp `css`/`layout.css`; text→`<span>`, vector→`<svg>`, ảnh→`<img data-raster>`) — scaffold để lặp, không phải code cuối |
| `figma_bridge_list_components` | **Inventory component** (phase 3): gom INSTANCE theo main component → `[{id,name,count,instanceIds}]` để nhận diện component lặp ("Button ×14") và dựng library |
